...
export interface Workouts {
  ...
  workoutsByWorkoutTemplateId: Record<AutomergeDocumentId<WorkoutTemplate>, AutomergeDocumentId<Workout>>;
}
...
export async function getWorkouts(offset: number, limit: number): Promise<[key: AutomergeDocumentId<Workout>, value: Workout][]> {
  const workouts = (await userDocument.current!.workouts()).doc();
  const startOffset = offset * limit;
  const endOffset = startOffset + limit - 1;
  const workoutIds = Object.keys(workouts.workoutsById).slice(startOffset, endOffset) as AutomergeDocumentId<Workout>[];
  const repo = getRepo();
  return await Promise.all(workoutIds.map(async id => [id, (await findDocument(id, repo)).doc()]));
}

export async function getWorkoutById(workoutId: AutomergeDocumentId<Workout>): Promise<Workout | null> {
...
export async function getActiveWorkoutByWorkoutTemplateId(workoutTemplateId: AutomergeDocumentId<WorkoutTemplate>): Promise<[key: AutomergeDocumentId<Workout>, value: Workout] | null> {
  const workouts = (await userDocument.current!.workouts()).doc();
...
export async function upsertWorkout(workoutUpdates: Workout, workoutId?: AutomergeDocumentId<Workout>) {
  const workouts = await userDocument.current!.workouts();
