...
import { updateDifferences, type GetKeyFn } from '$lib/shared/util/diff';
import { getGuid } from '$lib/shared/util/getGuid';
import { createNotification } from '$lib/shared/state/notifications.svelte';
...
export async function getExercises(
	...
	const exercises = (await userDocument.exercises()).doc();
...
export async function deleteExercise(exerciseGuid: AutomergeDocumentId<Exercise>) {
	const exercises = await userDocument.exercises();
...
export async function upsertExercise(
	...
	if (!exerciseDocumentId) {
		...
		exerciseDocument.change((e) => {
			updateDifferences(e, exercise, getGuid as GetKeyFn);
		});
	}
	return;
}
...
if (browser) {
	const checkForNewRelease =
		...
		lastExerciseRelease.value.updatedAt < Date.now() - 60 * 60 * 1000 * 24;
	...
	if (checkForNewRelease) {
		fetch(`${CORS_URL}${encodeURIComponent(`${RELEASES}?_=${Date.now()}`)}`)
			.then(async (releaseResponse) => {
				...
				let latestPublishedAt = lastExerciseRelease.value?.publishedAt ?? 0;
				let latestRelease;
				for (const release of releases) {
					if (release.draft || !release.published_at) {
						continue;
					}
					const publishedAt = new Date(release.published_at ?? 0);
					if (publishedAt.valueOf() < latestPublishedAt) {
						continue;
					}
					latestPublishedAt = publishedAt.valueOf();
