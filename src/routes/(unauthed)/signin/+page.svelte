<script lang="ts" module>
	...
	const createSuite = () =>
		create((data: Partial<SignUpForm> = {}, fields: string[]) => {
			if (!fields.length) {
				return;
			}
			only(fields);
...
<script lang="ts">
	...
	import { signIn } from '$lib/states/userDocumentIds.svelte';
...
<div class="flex flex-grow flex-col justify-end p-4 sm:justify-start">
	<div class="card mx-auto my-10 flex w-full flex-shrink flex-col sm:w-72">
		...
		<p>
			...
			<a href={`${base}/signup`} class="text-blue-500 underline">{m.auth_sign_up()}</a>
		</p>
